 ...
 ****************************************************************************/
   ...
   $testimonial="home";
	...
	if($flag == "active")
	{
		$Update	=	"UPDATE ".GROUP." SET `status`='0' WHERE `group_id`='$id'";
	...
	{
		$Update	=	"UPDATE ".GROUP." SET `status`='1' WHERE `group_id`='$id'";
		$Result	=	mysql_query($Update);
		$rflag	=	"success";
		$rType	=	"deactive";
	}
	elseif($flag == "delete")
	{
		$Delete	=	"DELETE FROM ".GROUP." WHERE `group_id`='$id'";
		$Result	=	mysql_query($Delete);
		$rflag	=	"success";
		$rType	=	"delete";
	}

	$Q_Check	= "SELECT * FROM ".GROUP."  WHERE (`group_name` LIKE '$SortList%') ORDER BY `group_id` ASC  ";
	...
	$Q_Check1	= "SELECT * FROM ".GROUP." WHERE (`group_name` LIKE '$SortList%') ORDER BY `group_id` ASC  ";
...
<div class="wrap">
	...
	<?php include("sidenav.php"); ?>			
		<div id="main">
			<div class="full_w">
				<div class="h_title">View Blood GROUP</div>
				 <?php
					  # Display the Result(Status) 
					  if($rflag == "success") 
					  { 
						  if($rType == "add")
							$rDisp	=	"Group Details Successfully Added";
						  elseif($rType == "deactive")
							$rDisp	=	"Group Details Successfully Actived";
						  elseif($rType == "active")
							$rDisp	=	"Group Details Successfully DeActived";
						  elseif($rType == "check")
							$rDisp	=	"Group Details Successfully turns to Popular";
						  elseif($rType == "uncheck")
							$rDisp	=	"Group Details Successfully cancel from Popular";
						  elseif($rType == "delete")
							$rDisp	=	"Group Details Deleted Successfully";
						  elseif($rType == "edit")
							$rDisp	=	"Group Details Edited Successfully";
				...
				<table>
					<thead>
						<tr>
						  ...
						  <th width="139" align="center" scope="col">Group Name</th>
					...
					<?php
							...
							$group_id = $F_Check['group_id'];
							$group_name = $F_Check['group_name'];
						?>
						<tr>
							<td class="align-center" align="center"><?php echo $r; ?></td>
							<td align="center"><?php echo $group_name; ?></td>
							<td align="center">
							<a style="text-decoration:none;" href="addbloodgroup.php?task=edit&amp;mid=<?php echo $F_Check['group_id'];?>&amp;page=<?php echo $page;?>" title="Edit">Edit</a>&nbsp;&nbsp;
							<a style="text-decoration:none;" href="viewbloodgroup.php?task=view&amp;flag=delete&amp;id=<?php echo $F_Check['group_id'];?>&amp;page=<?php echo $page;?>"  title="Delete">Delete</a>
							</td>
						</tr>
						<?php }  } else {?> 
						<tr>
							<td colspan="6" align="center"><h2>Group List Not Found</h2></td>
						</tr>
						<?php } ?>
					</tbody>

...
/*			$devlanguage=$_POST['package_time'];
			...
			$insert_query="INSERT INTO ".PACKAGE." (`package_name`,`package_dec`,`package_note` ,`package_st`) VALUES ('$package_name','$package_dec', '$package_note',  '$package_st')";
			$inder_ad_price=mysql_query($insert_query);
			$inser_id=mysql_insert_id();
			if($inser_id>0)
			{
				header("Location:viewpackage.php?task=general&log=succ");
			}					
		    ...
		    $update_query="UPDATE ".PACKAGE." SET `package_name`='$package_name',`package_dec`='$package_dec',`package_note` ='$package_note',`package_st` =  '$package_st' WHERE  `package_id`='$pack_id'";
			$update_ad_price=mysql_query($update_query);
			
			header("Location:viewpackage.php?task=edit&log=succ");
...
/*								$cqry = "select * from ".PRICE." where  `status`='1'";
				...
				<!--		</select>
					...
					<div class="element">
						<label for="name">Package Content<label><br />
						<textarea cols="80" id="editor1" name="package_dec" rows="10">
                        <?php echo $package_dec_edit; ?>
                        </textarea>
                        <script>
					...
					<!--<div class="element">
						<label for="name"><input type="checkbox" onclick="javascript:showdinote();" value="0"  id="ifnote" align="bottom" />&nbsp;If Note</label>						
						<label for="name"><input type="checkbox" onclick="javascript:showdinote();" value="0"  id="ifnonote" align="bottom" />&nbsp;No Need</label>					                        
					</div>-->
                    <div class="element" id="notediv">
						<label for="name">Enter the Note content</label>
						<input id="package_note" name="package_note" class="text" value="<?php echo $package_note_edit; ?>" />
					</div>
					<div class="element">
